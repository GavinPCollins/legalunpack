...
class ChatbotSessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "saves and returns legal references for assistant responses" do
    ...
    captured_prompt = nil
    captured_retriever_options = nil
    original_ai_call = AiClient.method(:call)
    original_retriever_call = LegalReferenceRetriever.method(:call)
    LegalReferenceRetriever.define_singleton_method(:call) do |query:, **options|
      captured_retriever_options = options
    ...
    assert_equal "VIC", captured_retriever_options[:jurisdiction]
    assert_equal "L1", json.dig("legal_references", 0, "label")
    assert_equal "Refunds and returns", json.dig("legal_references", 0, "title")
    assert_equal "Refunds and returns", json.dig("legal_references", 0, "citation")
    ...
    assert_equal "regulator_guidance", json.dig("legal_references", 0, "source_type")
